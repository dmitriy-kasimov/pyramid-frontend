...
const getResults = (time, countQueries, queries) => {
    ...
    time = new Date(2011, 0, 1, ...time.split(':'))

    const map = new Map();
    for(const query of queries){

        const [team, timeQuery, server, status] = query.split(' ')

        if(!map.has(team)){
            map.set(team, {[server]: {
                attempts: [ {timeQuery: new Date(2011, 0, 1, ...timeQuery.split(':')), status} ],
                scores: 0,
                penaltyTime: 0
            } })
        } else {
            ...
            const currentDay = hh < hhStart ? [2011, 0, 2] : [2011, 0, 1]
...
const str = "\"Team 1\" 01:10:00 A FORBIDDEN"
const [_, name, another] = str.split("\"")
console.log(`${name}: ${another.trimLeft()}`);